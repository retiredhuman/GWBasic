...
| `RUN [line]` | Run program (optionally from a line) |
| `LIST [n [END m]]` | List program lines |
| `LOAD file[,R]` | Load a .bas (`,R` = keep files open and run) |
| `SAVE file` / bare `SAVE` | Save program (bare SAVE reuses last name) |
| `NEW` | Erase program and variables |
| `AUTO [start[,inc]]` | Automatic line numbering |
| `DELETE a[-b]` | Delete lines |
| `EDIT n` / `LEDIT n` | Full-screen / one-line edit of a program line |
| `RENUM` | Renumber lines |
| `CLEAR` | Zero variables, close files (memory/stack args accepted, simulated) |
| `CONT` | Continue after STOP/Break |
| `TRACE [ON/OFF/line]` | Execution trace |
| `TOKEN` | Show tokenizer table |
